...
import { Users, LogOut, Bus, Server, GitCompare, Clock, TrendingUp,ServerCrash } from 'lucide-react';
...
export const Layout: React.FC = () => {
  ...
  const navigation = React.useMemo(() => {
    ...
    if (canViewBcoAlteracoes(user?.role)) items.push({ name: 'BCO Alterações', href: '/bco-alteracoes', icon: ServerCrash });
    return items;
  }, [user]);

  // Render with header and sidebar conditionally to prevent unmounting Outlet
  return (
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-gray-50 via-gray-50 to-gray-100 dark:from-gray-950 dark:via-gray-900 dark:to-neutral-900 transition-colors duration-500">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <main className={`flex-1 overflow-y-auto ${!isFullscreen ? 'p-4' : ''} custom-md:ml-0 transition-all duration-300`}>
          <div className={`${!isFullscreen ? 'mx-auto max-w-[1800px] w-full rounded-lg shadow-md bg-white/80 dark:bg-gray-900/95 backdrop-blur-lg p-4 border border-white/20 dark:border-yellow-500/20' : ''} transition-all duration-300`}>
            <Outlet />
          </div>
        </main>
      </div>
