...
export class UsersService {
  ...
  // ✅ MÉTODOS PARA AUTOLOGIN
  async findByAutoLoginToken(token: string): Promise<User | null> {
    console.log(`🔍 [FIND_BY_AUTOLOGIN_TOKEN] Buscando usuário por token de autologin`);
    ...
    if (user) {
      console.log(`✅ [FIND_BY_AUTOLOGIN_TOKEN] Usuário encontrado: ${user.email}`);
    } else {
      console.log(`❌ [FIND_BY_AUTOLOGIN_TOKEN] Usuário não encontrado ou autologin desabilitado`);
    }

    return user;
  }

  async enableAutoLogin(userId: string): Promise<{ token: string }> {
    console.log(`🔐 [ENABLE_AUTOLOGIN] Habilitando autologin para usuário: ${userId}`);

    const user = await this.findOne(userId);

    // Gerar novo token
    const token = user.generateAutoLoginToken();
    ...
    console.log(`✅ [ENABLE_AUTOLOGIN] Autologin habilitado para: ${user.email}`);

    return { token };
  }

  async disableAutoLogin(userId: string): Promise<void> {
    console.log(`🔒 [DISABLE_AUTOLOGIN] Desabilitando autologin para usuário: ${userId}`);

    const user = await this.findOne(userId);

    user.autoLoginEnabled = false;
    user.autoLoginToken = null;

    await this.usersRepository.save(user);

    console.log(`✅ [DISABLE_AUTOLOGIN] Autologin desabilitado para: ${user.email}`);
  }
}
