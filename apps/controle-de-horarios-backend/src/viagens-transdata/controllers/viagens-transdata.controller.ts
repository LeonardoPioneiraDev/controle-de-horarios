...
export class ViagensTransdataController {
  ...
  @ApiResponse({ 
    status: 502, 
    description: 'Erro na API Transdata' 
  })
  async buscarViagensPorData(
    ...
    try {
      ...
      this.logger.log(`[CONTROLLER] ✅ Retornando ${viagens.length} viagens para ${data}`);
      return viagens;
    } catch (error) {
      this.logger.error(`[CONTROLLER] ❌ Erro ao buscar viagens para ${data}:`, error.message);
      throw error;
    }
  }
  ...
  async buscarViagensComFiltros(
    ...
    this.logger.log(`[CONTROLLER] Buscando viagens filtradas para data: ${data}`, filtros);
    ...
    try {
      const resultado = await this.viagensService.buscarViagensComFiltros(data, filtros);
      this.logger.log(`[CONTROLLER] ✅ Retornando ${resultado.data.length}/${resultado.total} viagens filtradas`);
      return resultado;
    } catch (error) {
      this.logger.error(`[CONTROLLER] ❌ Erro ao buscar viagens filtradas:`, error.message);
      throw error;
    }
  }

  /**
   * ✅ ROTA 3: SINCRONIZAÇÃO MANUAL
   */
  @Post('sincronizar/:data')
  @Roles(UserRole.ANALISTA) // ✅ Requer nível analista ou superior para sincronizar
  @ApiOperation({ 
    summary: 'Sincronizar viagens manualmente',
    description: 'Força a sincronização das viagens com a API Transdata. Acesso: ANALISTA ou superior.'
  })
  @ApiParam({ 
    name: 'data', 
    description: 'Data no formato YYYY-MM-DD', 
  ...
  async sincronizarViagensPorData(@Param('data') data: string): Promise<{
    ...
    try {
      const resultado = await this.viagensService.sincronizarViagensPorData(data);
      ...
      this.logger.log(`[CONTROLLER] ✅ Sincronização manual concluída:`, response);
      return response;
    } catch (error) {
      this.logger.error(`[CONTROLLER] ❌ Erro na sincronização manual:`, error.message);
      throw error;
    }
  }
  ...
  async obterCodigosLinha(@Param('data') data: string): Promise<{
    data: string;
    linhas: string[];
    total: number;
  }> {
    this.logger.log(`[CONTROLLER] Buscando códigos de linha para data: ${data}`);
    ...
    try {
      ...
      this.logger.error(`[CONTROLLER] ❌ Erro ao buscar códigos de linha:`, error.message);
      throw error;
    }
  }
  ...
  async obterServicos(@Param('data') data: string): Promise<{
    data: string;
    servicos: number[];
    total: number;
  }> {
    ...
    try {
      ...
      this.logger.error(`[CONTROLLER] ❌ Erro ao buscar serviços:`, error.message);
      throw error;
    }
  }
  ...
  async verificarStatusDados(@Param('data') data: string): Promise<{
    data: string;
    ...
    try {
      const existemDados = await this.viagensService.verificarDadosExistem(data);
      ...
      const viagens = await this.viagensService.buscarViagensPorData(data);
      const ultimaSincronizacao = viagens.length > 0 
        ? viagens[0].ultimaSincronizacao?.toISOString() 
        : null;
      ...
      this.logger.error(`[CONTROLLER] ❌ Erro ao verificar status:`, error.message);
      throw error;
    }
  }
  ...
  @ApiOperation({ 
    ...
    description: 'Testa a conectividade com a API Transdata. Acesso: GERENTE ou superior.'
  })
  @ApiResponse({ 
    status: 200, 
  ...
  async testarConexaoApi(): Promise<{
    ...
    try {
      ...
      this.logger.error(`[CONTROLLER] ❌ Erro no teste de conexão:`, error.message);
      throw error;
    }
  }
  ...
  @ApiOperation({ 
    ...
    description: 'Retorna configurações e estatísticas da API. Acesso: GERENTE ou superior.'
  })
  @ApiResponse({ 
    status: 200, 
  ...
  async obterEstatisticasApi(): Promise<any> {
    ...
    try {
      ...
      this.logger.error(`[CONTROLLER] ❌ Erro ao obter estatísticas:`, error.message);
      throw error;
    }
  }
